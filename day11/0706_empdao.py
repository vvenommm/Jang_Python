...
class EmpDao:
    ...
    def myinsert(self, e_name, sex, addr):
        sql = f"insert into emp(e_name, sex, addr) values('{e_name}', '{sex}', '{addr}')"
        cnt = self.curs.execute(sql)
        
        self.conn.commit()
        return cnt
    
    def myupdate(self, e_id, e_name, addr):
        sql=f"update emp set e_name = '{e_name}', addr = '{addr}' where e_id = '{e_id}'"
        cnt = self.curs.execute(sql)
